...
ISR(TIMER1_COMPA_vect)
{
  ...
  case 1: // Initialise transmission
    if(radiostatus==1)
    {
      ...
      lockvariables=1;
      sprintf(txstring, "$$$$$PAVA,%i,%02d:%02d:%02d,%s%i.%05ld,%s%i.%05ld,%ld,%d,%i",count, hour, minute, second,lat < 0 ? "-" : "",lat_int,lat_dec,lon < 0 ? "-" : "",lon_int,lon_dec, maxalt,sats,errorstatus);
